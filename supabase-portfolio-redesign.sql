...
create policy "Users can manage their portfolio watchlist"
  on public.portfolio_watchlist
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
...
create policy "Users can manage their portfolio AI settings"
  on public.portfolio_ai_settings
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
